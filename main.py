    ...
    #  print(pc)
        ...
        for pc in all_pcs:
            if int(pc_choice) == int(pc[0]):
                ...
                break
        if pc_chosen == True:
            display_pc(chosen_pc)
        else:
            print("Select a corresponding PC number")

def create_new_pc(login_input):
    Pc.create_table()
...
def create_part(part_type, pc):
    ...
    if part_type == "Motherboard" or part_type == "CPU":
        print("What is the motherboard's socket?")
        part_chipset = input()
        while part_chipset == "":
            print("Field cannot be empty. Please enter a valid input:")
            part_chipset = input()
...
def pc_status(pc_parts):
    ...
    while part_choice != 7:
        ...
        if part_choice == 1:
            ...
            print(f"You currently have a {psu_power}W Power Supply")
            print(f"Make sure the Power Supply is at least {total_power - psu_power}W more powerful!")
        elif part_choice == 3:
            total_price = 0
            for part in pc_parts:
                total_price += part[4]
